			...
			for (TestCaseVo l1 : testCase.getResultList()) {
				...
				if (l1.getMethodName().equals(methodName)) {
					for (i = 1; i < ConstantUtils.RETRY_FAIL_SHORT_COUNT + 1; i++) {
						logger.info("AppiumExecutor TestCaseVo is "  +  l1.toString());
						try {
							executeFunction2(l1);
...
//							shotRetryFlag = true;
//							e.retryLog(logger);
							logger.info(l1.getNo() + " " +l1.getMethodName() + " " + l1.getDes() + " run failed " + i +  " times ");
							Thread.sleep(ConstantUtils.RETRY_SHORT_INTERVAL);
							continue;
						} 
...
//		l1.getFindByType(),l1.getObj(),l1.getFunction(),l1.getValue()
	...
	public void executeFunction2(TestCaseVo testCaseVo) {
		switch (testCaseVo.getFunction()) {
			case "click" :
				findElementByType(testCaseVo).click();
				break;
			case "input" :
				findElementByType(testCaseVo).click();
				findElementByType(testCaseVo).sendKeys(testCaseVo.getValue());;
				break;
//			case "inputPwd" :
//				String enabledFlag;
//				do {
...
////				((JavascriptExecutor)driver).executeScript("document.getElementById(\"em\").type ='text';");
	...
	public WebElement findElementByType(TestCaseVo testCaseVo) {
		switch (testCaseVo.getFindByType()) {
		case "By.xpath":
			return driver.findElementByXPath(testCaseVo.getObj());
		case "By.id":
			return driver.findElementById(testCaseVo.getObj());
		case "By.name":
			return driver.findElementByName(testCaseVo.getObj());
		case "By.linkText":
			return driver.findElementByLinkText(testCaseVo.getObj());
		case "By.partialLinkText":
			return driver.findElementByPartialLinkText(testCaseVo.getObj());
		case "By.className":
			return driver.findElementByClassName(testCaseVo.getObj());
		case "By.cssSelector":
			return driver.findElementByCssSelector(testCaseVo.getObj());	
		default:
				break;
		}
		return null;
